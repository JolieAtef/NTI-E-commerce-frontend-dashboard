...
export class UsersService {
    ...
    getUsers(limit: number = 50, page: number = 1, sort: string = '-createdAt', search: string, role: string = 'admin'): Observable<any> {
      return this._HttpClient.get(`${this.HostName}${this.routeName}?limit=${limit}&page=${page}&sort=${sort}&search=${search}&role=${role}&fields=-password`,
    ...
    updateUser(userId: string, formData: any): Observable<any> {
      return this._HttpClient.put(`${this.HostName}${this.routeName}/${userId}`,
    ...
    updateUserPassword(userId: string, formData: any): Observable<any> {
      return this._HttpClient.put(`${this.HostName}${this.routeName}/${userId}/changePassword`,
        formData, { headers: { authorization: `Bearer ${localStorage.getItem('user')}` } })
    }
  
    deleteUser(userId: string): Observable<any> {
      return this._HttpClient.delete(`${this.HostName}${this.routeName}/${userId}`,
        { headers: { authorization: `Bearer ${localStorage.getItem('user')}` } })
    }
  }
